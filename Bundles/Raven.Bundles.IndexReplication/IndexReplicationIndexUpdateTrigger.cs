...
namespace Raven.Bundles.IndexReplication
{
	public class IndexReplicationIndexUpdateTrigger : AbstractIndexUpdateTrigger
	{
		...
		public override AbstractIndexUpdateTriggerBatcher CreateBatcher(string indexName)
		{
			...
			var defaultSettingDoc = Database.Get("Raven/IndexReplication/" + IndexReplicationDestination.DEFAULT_SETTING_DOCUMENT, null);

			bool? initialized = false;
			if (defaultSettingDoc != null)
			{
				var defaultSetting = defaultSettingDoc.DataAsJson.JsonDeserialization<IndexReplicationDestination>();
				initialized = defaultSetting.Initialized;
			}

			var destination = document.DataAsJson.JsonDeserialization<IndexReplicationDestination>();
			destination.Initialized = destination.Initialized ?? initialized;
		...
		public class ReplicateToSqlIndexUpdateBatchCommandBatcher : AbstractIndexUpdateTriggerBatcher
		{
			...
			{
				int paraIndex = cmd.Parameters.Count;
				var pkParam = cmd.CreateParameter();
				pkParam.ParameterName = GetParameterName("entryKey", paraIndex);
				...
				foreach (var mapping in destination.ColumnsMapping)
				{
					paraNextIndex = cmd.Parameters.Count;
					var parameter = cmd.CreateParameter();
					parameter.ParameterName = GetParameterName(mapping.Key, paraNextIndex);
			...
			{
				//only execute if it is initializer
				if (destination.Initialized == true)
				{
					var parameter = cmd.CreateParameter();
					parameter.ParameterName = GetParameterName("entryKey", cmd.Parameters.Count);
			...
			private string GetParameterName(string paramName, int index)
			{
				if (connection is SqlConnection)
					return "@" + paramName + index.ToString();
				return ":" + paramName + index.ToString();
			}

			private void ExecuteBatch()
			{
				if (cmd.Parameters.Count > 512)
				{
					using (cmd)
					{
